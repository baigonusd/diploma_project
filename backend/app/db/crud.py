...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(
        email=user.email, hashed_password=fake_hashed_password, fio=user.fio, iin=user.iin, tel=user.tel)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def update_user(db: Session, user: schemas.UserUpdate, user_id: int):
    ...
    if Hasher.verify_password(user.old_password, existing_user.hashed_password) == "OK":
        ...
        existing_user.fio = user.fio
        existing_user.iin = user.iin
        existing_user.tel = existing_user.tel
        existing_user.scan_udv = existing_user.scan_udv
